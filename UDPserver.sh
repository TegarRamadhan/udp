...
time_reboot(){
  print_center -ama "${a92:-MEMULAI ULANG VPS AKTIF} $1 ${a93:-DETIK}"
  REBOOT_TIMEOUT="$1"
...
check_sistem(){
  fail(){
    ...
    echo -e "\e[1m\e[33m${a94:-skrip ini tidak kompatibel dengan sistem operasi Anda}\e[0m"
...
if [[ ! -e $udp_file/UDPserver.sh ]]; then
  ...
  check_sistem
	...
	rm $(pwd)/$0 &> /dev/null
	title "${a102:-PEMASANGAN SELESAI}"
	print_center -ama "${a103:-Gunakan perintah\nudp\untuk menjalankan menu}"
	msg -bar
	time_reboot 10
fi
...
data_user(){
	...
	[[ -z "$(echo "${cat_users}"|head -1)" ]] && print_center -verm2 "${a96:-TIDAK ADA PENGGUNA SSH TERDAFTAR}" && return 1
...
limiter(){
	...
	ltr(){
		...
		for i in `atq|awk '{print $1}'`; do
			if [[ ! $(at -c $i|grep 'limitador.sh') = "" ]]; then
				...
				print_center -verd "${a68:-pembatas berhenti}"
				enter
				return
			fi
		done
    print_center -ama "${a69:-SET PEMBATAS}"
    msg -bar
    print_center -ama "${a70:-Blokir pengguna saat melebihi}"
    print_center -ama "${a71:-jumlah maksimum koneksi}"
    msg -bar
    unset opcion
    while [[ -z $opcion ]]; do
      msg -nama " ${a72:-Jalankan limitor setiap}: "
      read opcion
      if [[ ! $opcion =~ $numero ]]; then
        del 1
        print_center -verm2 " ${a73:-Hanya nomor yang diperbolehkan}"
        sleep 2
        del 1
        unset opcion && continue
      elif [[ $opcion -le 0 ]]; then
        del 1
        print_center -verm2 "${a74:-waktu minimal 1 menit}"
        sleep 2
        del 1
        unset opcion && continue
      fi
      del 1
      echo -e "$(msg -nama " ${a75:-Jalankan limitor setiap}:") $(msg -verd "$opcion ${a76:-Menit}")"
      echo "$opcion" > ${udp_file}/limit
    done

    msg -bar
    print_center -ama "${a77:-Pengguna yang diblokir oleh pembatas\nakan dibuka kuncinya secara otomatis\n(masukkan 0 untuk buka kunci manual)}"
    msg -bar

    unset opcion
    while [[ -z $opcion ]]; do
      msg -nama " ${a78:-Buka blokir pengguna setiap}: "
      read opcion
      if [[ ! $opcion =~ $numero ]]; then
        tput cuu1 && tput dl1
        print_center -verm2 " ${a73:-Hanya nomor yang diperbolehkan}"
      ...
      tput cuu1 && tput dl1
      [[ $opcion -le 0 ]] && echo -e "$(msg -nama " ${a79:-membuka kunci}:") $(msg -verd "${a80:-manual}")" || echo -e "$(msg -nama " ${a78:-Buka blokir pengguna setiap}:") $(msg -verd "$opcion ${a76:-menit}")"
      echo "$opcion" > ${udp_file}/unlimit
    done
		nohup ${udp_file}/limitador.sh &>/dev/null &
    msg -bar
		print_center -verd "${a81:-menjalankan pembatas}"
		enter	
	}
	...
	l_exp(){
    	...
    	if [[ -z "$l_cron" ]]; then
      		...
      		print_center -verd "${a82:-pembatas kedaluwarsa terjadwal\nakan dijalankan setiap hari pada pukul 1 pagi\nsesuai dengan waktu yang dijadwalkan di server}"
    	else
      		sed -i '/limitador.sh --ssh/d' /var/spool/cron/crontabs/root
      		print_center -verm2 "${a83:-limiter kedaluwarsa berhenti}"   
    	fi
      enter
      return
	}
	...
	log(){
		clear
		msg -bar
		print_center -ama "${a84:-PENDAFTARAN PEMBATASAN}"
		msg -bar
		[[ ! -e ${udp_file}/limit.log ]] && touch ${udp_file}/limit.log
		if [[ -z $(cat ${udp_file}/limit.log) ]]; then
			print_center -ama "${a85:-tidak ada register pembatas}"
			msg -bar
			sleep 2
			return
		fi
		...
		msg -bar
		print_center -ama "►► ${a86:-Tekan enter untuk melanjutkan atau} ◄◄"
		print_center -ama "►► ${a87:- untuk membersihkan rekaman} ◄◄"
		read opcion
	...
	clear
	msg -bar
	print_center -ama "${a11:-PEMBATASAN AKUN}"
	msg -bar
	menu_func "${a64:-PEMBATASAN MULTI-LOGIN}" "${a65:-PEMBATASAN KADALUARSA} $lim_e" "${a66:-LOG PEMBATASAN}"
	back
	msg -ne " ${a67:-pilihan}: "
	read opcion
	case $opcion in
...
detail_user(){
	...
	if [[ -z ${usuarios_ativos[@]} ]]; then
		msg -bar
		print_center -verm2 "${a62:-Tidak ada pengguna terdaftar}"
		msg -bar
		sleep 3
		return
	else
		msg -bar
		print_center -ama "${a63:-DETAIL PENGGUNA}"
		msg -bar
	fi
	data_user
	msg -bar
...
block_user(){
  ...
  usuarios_ativos=('' $(mostrar_usuarios))
  msg -bar
  print_center -ama "${a9:-LOCK/UNLOCK PENGGUNA}"
  msg -bar
  data_user
  back
  ...
  unset selection
  while [[ ${selection} = "" ]]; do
    echo -ne "\033[1;37m ${a59:-Pilih}: " && read selection
    del 1
  done
  [[ ${selection} = "0" ]] && return
  ...
  [[ -z $usuario_del ]] && {
    msg -verm "${a54:-Kesalahan, Pengguna Tidak Valid}"
    msg -bar
    return 1
  }
  ...
  msg -nama "   ${a48:-Pengguna}: $usuario_del >>>> "
  ...
  if [[ $(passwd --status $usuario_del|cut -d ' ' -f2) = "P" ]]; then
    ...
    sleep 2
    msg -verm2 "${a60:-terkunci}"
  else
  	usermod -U $usuario_del
  	sleep 2
  	msg -verd "${a61:-tidak terkunci}"
  fi
  msg -bar
  sleep 3
}
...
renew_user_fun(){
  ...
  if chage -E $valid $1 ; then
  	print_center -ama "${a100:-Berhasil Memperbarui Pengguna}"
  else
  	print_center -verm "${a101:-Kesalahan, Pengguna tidak Diperbarui
}"
  fi
}
...
renew_user(){
  ...
  usuarios_ativos=('' $(mostrar_usuarios))
  msg -bar
  print_center -ama "${a8:-PERBARUI PENGGUNA}"
  msg -bar
  data_user
  back

  print_center -ama "${a52:-Tulis atau Pilih Pengguna}"
  msg -bar
  unset selection
  while [[ -z ${selection} ]]; do
    msg -nazu "${a53:-Pilih Opsi}: " && read selection
    del 1
  done
  ...
  while true; do
    msg -ne "${a58:-Waktu Durasi Baru}: $useredit"
    read -p ": " diasuser
    if [[ -z "$diasuser" ]]; then
      echo -e '\n\n\n'
      err_fun 7 && continue
...
droppids(){
  ...
  loginsukses='Autentikasi kata sandi berhasil'
...
remove_user(){
	...
	usuarios_ativos=('' $(mostrar_usuarios))
	msg -bar
	print_center -ama "${a7:-HAPUS PENGGUNA}"
	msg -bar
	data_user
	back
	...
	print_center -ama "${a55:-Pengguna yang Dipilih}: $usuario_del"
	rm_user "$usuario_del"
  if [[ $msj = 0 ]] ; then
    print_center -verd "[${a56:-DIHAPUS}]"
  else
    print_center -verm "[${a57:-tidak dihapus}]"
  fi
  enter
}
...
new_user(){
  ...
  usuarios_ativos=('' $(mostrar_usuarios))
  msg -bar
  print_center -ama "${a6:-BUAT PELANGGAN}"
  msg -bar
  data_user
  back

  while true; do
    msg -ne " ${a41:-Nama belakang}: "
    read nomeuser
  ...
  while true; do
    msg -ne " ${a42:-Kata Sandi Pengguna}"
    read -p ": " senhauser
  ...
  msg -bar
  if [[ $msj = 0 ]]; then
    print_center -verd "${a45:-Pengguna Berhasil Dibuat}"
  else
    print_center -verm2 "${a46:-Kesalahan, Pengguna tidak dibuat}"
    enter
    return 1
  fi
  msg -bar
  msg -ne " ${a47:-IP server}: " && msg -ama "    $ip_publica"
  msg -ne " ${a48:-Pengguna}: " && msg -ama "            $nomeuser"
  msg -ne " ${a49:-Kata sandi}: " && msg -ama "         $senhauser"
  msg -ne " ${a50:-Durasi hari}: " && msg -ama "   $diasuser"
  msg -ne " ${a44:-Batas Koneksi}: " && msg -ama " $limiteuser"
  msg -ne " ${a51:-Tanggal habis tempo}: " && msg -ama "$(date "+%F" -d " + $diasuser days")"
  enter
}
...
download_udpServer(){
	msg -nama "        ${a30:-Mengunduh server UDP} ....."
...
EOF
	...
	msg -nama "        ${a31:-Menjalankan layanan server UDP} ....."
...
install_UDP(){
  ...
  exclude
	...
	if [[ $(type -p udpServer) ]]; then
		...
		msg -bar3
		if [[ $(systemctl is-active UDPserver) = 'active' ]]; then
			print_center -verd "${a17:-pemasangan selesai}"
		else
			print_center -verm2 "${a18:-gagal menjalankan layanan}"
		fi
	else
		echo
		print_center -ama "${a19:-Gagal mengunduh udpServer}"
	fi
	enter	
}

uninstall_UDP(){
	title "${a32:-Uninstall UDPserver}"
	read -rp " $(msg -ama "${a33:-INGIN MENGHAPUS UDPserver? [S/N]}:") " -e -i S UNINS
	...
	rm -rf /usr/bin/udpServer
	del 1
	print_center -ama "${a34:-uninstall penuh!}"
	enter
}
...
reset(){
	if [[ $(systemctl is-active UDPserver) = 'active' ]]; then
		...
		if [[ $(systemctl is-active UDPserver) = 'active' ]]; then
			systemctl enable UDPserver &>/dev/null
			print_center -verd "${a36:-Server UDP dimulai!}"
		else
			print_center -verm2 "${a37:-gagal memulai server UDP!}"
		fi	
	fi
	enter
}
...
menu_udp(){
	title "${a1:-This UDP Script Made By} BY @Ansy89 & @CieeeeKeepooo"
	print_center -ama 'UDPserver by InetSolidaritas'
	print_center -ama 'UDPclient Android '
	msg -bar
    ...
    if [[ ! $port = "" ]]; then
      ...
      print_center -ama "${a2:-PORT YANG DIKECUALIKAN} $port"
      msg -bar
    fi
    ...
    msg -bar
		...
		echo " $(msg -verd "[1]") $(msg -verm2 '>') $(msg -verm2 "${a3:-HAPUS server UDP}")"
		echo -e " $(msg -verd "[2]") $(msg -verm2 '>') $(msg -azu "${a4:-MULAI/HENTIKAN server UDP}") $estado"
    echo " $(msg -verd "[3]") $(msg -verm2 '>') $(msg -azu "${a5:-HAPUS SCRIPT}")"
		msg -bar3
    echo " $(msg -verd "[4]") $(msg -verm2 '>') $(msg -azu "Bahasa")"
    msg -bar3
		echo " $(msg -verd "[5]") $(msg -verm2 '>') $(msg -verd "${a6:-BUAT PELANGGAN}")"
		echo " $(msg -verd "[6]") $(msg -verm2 '>') $(msg -verm2 "${a7:-Hapus Pelanggan}")"
		echo " $(msg -verd "[7]") $(msg -verm2 '>') $(msg -ama "${a8:-Perbarui Pelanggan}")"
		echo " $(msg -verd "[8]") $(msg -verm2 '>') $(msg -azu "${a9:-KUNCI/BUKA PELANGGAN}")"
		echo " $(msg -verd "[9]") $(msg -verm2 '>') $(msg -blu "${a10:-Detail Pelanggan}")"
		echo " $(msg -verd "[10]") $(msg -verm2 '>') $(msg -azu "${a11:-AKUN TERBATAS}")"
		msg -bar3
    print_center -ama "${a12:-PENGECUALIAN PELABUHAN}"
    msg -bar3
    echo " $(msg -verd "[11]") $(msg -verm2 '>') $(msg -verd "${a13:-TAMBAHKAN PORT KE DAFTAR PENGECUALIAN}")"
		num=11
    if [[ ! $port = "" ]]; then
      echo " $(msg -verd "[12]") $(msg -verm2 '>') $(msg -verm2 "${a14:-HAPUS PORT DARI DAFTAR PENGECUALIAN}")"
      num=12
    fi
    a=x; b=1
	else
		echo " $(msg -verd "[1]") $(msg -verm2 '>') $(msg -verd "${a15:-INSTALL UDPserver}")"
		num=1; a=1; b=x
	fi
	back
